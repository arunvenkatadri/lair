...
**Public discovery.** Sencha will initially run as Arun Venkatadri's personal project on a standalone site, with a blog and a research-style report for each completed experiment. Every experiment must aim to produce results the ROS and Copper communities can learn from: state the intended lesson for each community before implementation and report the supported lessons afterward, including negative results. The reports will build a public record of methods, results, and reproducible artifacts toward the early-2027 paper. Extelligence placement follows traction. See the [site and publishing plan](WEBSITE_POSITIONING.md) and [experiment report template](EXPERIMENT_REPORT_TEMPLATE.md).
