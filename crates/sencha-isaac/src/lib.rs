//! Sencha Isaac — simulation bridge interface.
//!
//! Defines the `SimBridge` trait for connecting Sencha task graphs to
//! simulation environments (NVIDIA Isaac Sim, Gazebo, etc.).

use sencha_core::prelude::SenchaResult;

/// Bridge between Sencha and an external simulator.
///
...
pub trait SimBridge {
    ...
    fn step(&mut self, dt_ns: u64) -> SenchaResult<()>;

    /// Send a command to the simulator on the given channel.
    fn send_command(&mut self, channel: &str, data: &[u8]) -> SenchaResult<()>;

    /// Receive sensor data from the simulator on the given channel.
    fn receive_sensor(&mut self, channel: &str) -> SenchaResult<Vec<u8>>;

    /// Reset the simulation to its initial state.
    fn reset(&mut self) -> SenchaResult<()>;
...
impl SimBridge for MockSimBridge {
    ...
    fn receive_sensor(&mut self, _channel: &str) -> SenchaResult<Vec<u8>> {
        Ok(Vec::new())
    }
