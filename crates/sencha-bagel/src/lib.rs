//! Sencha Bagel — structured logging and telemetry.
//!
//! Re-exports the MCAP logging backend from `cu29-unifiedlog` and provides
//! convenience helpers for setting up Sencha logging with sensible defaults.
...
/// Set up Sencha logging with MCAP defaults (LZ4 compression).
///
/// This is a convenience wrapper around `basic_copper_setup` that
/// uses good defaults for Sencha applications.
pub fn basic_sencha_setup(
    log_path: &Path,
    ...
    let logger = UnifiedLoggerBuilder::new()
        ...
        .build()
        .map_err(|e| with_cause("Failed to create Sencha logger", e))?;
