...
use cu29_unifiedlog::{
    SectionHeader, SectionStorage, UnifiedLogRead, UnifiedLogWrite,
    memmap::{LogPosition, MmapUnifiedLogger, MmapUnifiedLoggerBuilder, MmapUnifiedLoggerRead},
};
    ...
    L: UnifiedLogWrite<S> + 'static,
{
    ...
    log_reader: MmapUnifiedLoggerRead,
...
fn build_read_logger(log_base: &Path) -> CuResult<MmapUnifiedLoggerRead> {
    let logger = MmapUnifiedLoggerBuilder::new()
        .file_base_name(log_base)
        .build()
        .map_err(|e| CuError::new_with_cause("Failed to open unified log", e))?;
    let MmapUnifiedLogger::Read(dl) = logger else {
...
fn read_section_at(
    log_reader: &mut MmapUnifiedLoggerRead,
    pos: LogPosition,
    ...
    TF: Fn(&crate::copperlist::CopperList<P>) -> Option<CuTime>,
{
    let logger = MmapUnifiedLoggerBuilder::new()
        ...
        .build()
        .map_err(|e| CuError::new_with_cause("Failed to open unified log", e))?;
    let MmapUnifiedLogger::Read(mut dl) = logger else {
        return Err(CuError::from("Expected read-only unified logger"));
    };
