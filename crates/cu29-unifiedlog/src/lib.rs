...
#[cfg(feature = "mcap")]
pub mod mcap_backend;

// ── Feature-gated compat aliases ────────────────────────────────────
// When the `mcap` feature is on, the default storage/writer/builder types
// point to the MCAP backend. The mmap backend remains available via
// `cu29_unifiedlog::memmap::*`.

#[cfg(all(feature = "std", feature = "mcap"))]
mod compat {
    pub use crate::mcap_backend::McapLoggerBuilder as UnifiedLoggerBuilder;
    pub use crate::mcap_backend::McapSectionStorage as DefaultSectionStorage;
    pub use crate::mcap_backend::McapUnifiedLogger as UnifiedLogger;
    pub use crate::mcap_backend::McapUnifiedLoggerWrite as UnifiedLoggerWrite;

    // Keep mmap read types accessible for log reading
...
#[cfg(all(feature = "std", not(feature = "mcap")))]
mod compat {
    pub use crate::memmap::LogPosition;
    pub use crate::memmap::MmapSectionStorage as DefaultSectionStorage;
